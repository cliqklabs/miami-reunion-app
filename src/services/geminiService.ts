 ...
 * @param styleName The style name (e.g., "Kingpin").
 * @returns The fallback prompt string.
 */
function getFallbackPrompt(styleName: string): string {
    const stylePrompts: Record<string, string> = {
        "Kingpin": "Create a professional photograph of the person as a sophisticated Miami business executive in an expensive suit with a confident pose and luxury Miami backdrop.",
        "Vice": "Create a photograph of the person in the classic 1980s Miami Vice style with pastel suit, aviator sunglasses, and neon lights.",
        "Player": "Create a photograph of the person as a Miami nightlife legend with flashy designer clothes, club lighting, and gold chains.",
        "Tourist": "Create a photograph of the person in casual Miami vacation style with Hawaiian shirt, sunglasses, and beach backdrop."
    };

    return stylePrompts[styleName] || `Create a photorealistic image of the person in Miami style: ${styleName}.`;
}
...
function extractStyleName(prompt: string): string | null {
    const styleNames = ["Kingpin", "Vice", "Player", "Tourist"];
 ...
 */
export async function generateMiamiImage(imageDataUrl: string, prompt: string): Promise<string> {
